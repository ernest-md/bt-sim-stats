...
drop function if exists public.get_global_featured_matches_v1(timestamptz, timestamptz);

create or replace function public.get_global_featured_matches_v1(
  p_start_at timestamptz default null,
  p_end_at timestamptz default null
)
returns table (
...
begin
  ...
  select
    ...
    pr.username as profile_username,
    pr.display_name as profile_display_name,
    coalesce(pr.avatar_url, '') as profile_avatar_url,
...
revoke all on function public.get_global_featured_matches_v1(timestamptz, timestamptz) from public;
grant execute on function public.get_global_featured_matches_v1(timestamptz, timestamptz) to authenticated;
